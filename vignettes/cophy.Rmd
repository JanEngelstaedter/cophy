...
In some cases it might be useful to plot only the extant species of a cophylogeny. In this case, the host and parasite tree can not be plotted together as the historical host-parsite associations may no longer be correct. Instead, by calling the `prune_Cophylo` function, each of the pruned host and parasite trees can be plotted separatly with connector lines expressing the terminal host-parasite associations. This plotting functionality was outsourced to the `phytools` package [@Revell2012]. An example of a pruned cophylogeny can be seen bellow:

```{r}
  set.seed(1)
  coph <- rcophylo(HTree = htree)
  prune_Cophylo(cophy = coph)
```

This function can also return the pruned host and parasite tree, along with a data.frame containing the association status for each host and parasite tip when the parameter `plot` is set to `FALSE`. The default value for `plot = TRUE`. 

```{r}
  prune_Cophylo(cophy = coph, plot = FALSE)
```
