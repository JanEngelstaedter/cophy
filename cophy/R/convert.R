...
convert_HPBranchesToCophylo <- function(HBranches, PBranches, prune.extinct = FALSE) {
  ...
  if (prune.extinct == TRUE) {
    ...
    Hphy <- list(edge = cbind(rHBranches$nodeBirth, rHBranches$nodeDeath),
                 edge.length = rHBranches$tDeath - rHBranches$tBirth,
                 tip.label = paste("t", 1:(1 + nHBranches / 2), sep = ""),
                 root.edge = Hroot.edge, nAlive = nHAlive)
    class(Hphy) <- "phylo"
...
convert_PQBranchesToPhylo<-function(P.PBranches,Q.PBranches,prune.extinct=FALSE)
{
  ...
  if (prune.extinct == TRUE) {
    ...
    for (i in 1:P.nPBranches) {
      if ((P.prunedPBranches$nodeDeath[i] > P.nPAlive) &&
          (length(P.prunedPBranches[P.prunedPBranches$nodeBirth == P.prunedPBranches$nodeDeath[i], 1]) == 1)) {
        # this branch does not terminate in a tip and also not in two new branches
    ...
    # if a new root branch has been formed, mark this for deletion as well P:
    for (i in 1:P.nPBranches) {
      if ((P.prunedPBranches$nodeBirth[i] > 0) &&
          (length(P.prunedPBranches[P.prunedPBranches$nodeBirth == P.prunedPBranches$nodeBirth[i], 1]) < 2))
  ...
  if (prune.extinct == TRUE) {
    ...
    for (i in 1:Q.nPBranches) {
      if ((Q.prunedPBranches$nodeDeath[i] > Q.nPAlive) &&
          (length(Q.prunedPBranches[Q.prunedPBranches$nodeBirth == Q.prunedPBranches$nodeDeath[i], 1]) == 1)) {
        # this branch does not terminate in a tip and also not in two new branches
    ...
    # if a new root branch has been formed, mark this for deletion as well P:
    for (i in 1:Q.nPBranches) {
      if ((Q.prunedPBranches$nodeBirth[i] > 0) &&
          (length(Q.prunedPBranches[Q.prunedPBranches$nodeBirth == Q.prunedPBranches$nodeBirth[i], 1]) < 2)) {
  ...
  if (prune.extinct == TRUE) {
    P.Pphy <- list(edge = cbind(P.prunedPBranches$nodeBirth, P.prunedPBranches$nodeDeath),
                   edge.length = P.prunedPBranches$tDeath - P.prunedPBranches$tBirth,
                   tip.label = paste("t", 1:P.nPAlive, sep = ""), root.edge = P.Proot.edge,
                   root.time = P.Proot.time, nAlive = P.nPAlive,
                   Hassoc = P.prunedPBranches$Hassoc, root.Hassoc = P.Proot.Hassoc)
    class(P.Pphy)	<- "phylo"
    P.Pphy$Nnode	<- P.nPAlive - 1

    Q.Pphy <- list(edge = cbind(Q.prunedPBranches$nodeBirth, Q.prunedPBranches$nodeDeath),
                   edge.length = Q.prunedPBranches$tDeath - Q.prunedPBranches$tBirth,
                   tip.label = paste("t", 1:Q.nPAlive, sep = ""), root.edge = Q.Proot.edge,
                   root.time = Q.Proot.time, nAlive = Q.nPAlive,
                   Hassoc = Q.prunedPBranches$Hassoc, root.Hassoc = Q.Proot.Hassoc)
    class(Q.Pphy)	<- "phylo"
    Q.Pphy$Nnode	<- Q.nPAlive - 1
  } else { # extinct taxa included:
    P.Pphy <- list(edge = cbind(P.rPBranches$nodeBirth, P.rPBranches$nodeDeath),
                   edge.length = P.rPBranches$tDeath - P.rPBranches$tBirth,
                   tip.label = paste("t", 1:(1 + P.nPBranches / 2), sep = ""),
                   root.edge = P.Proot.edge, root.time = P.Proot.time, nAlive = P.nPAlive,
                   Hassoc = P.rPBranches$Hassoc, root.Hassoc=P.Proot.Hassoc)
    class(P.Pphy)	<- "phylo"
    P.Pphy$Nnode	<- P.nPBranches / 2

    Q.Pphy <- list(edge = cbind(Q.rPBranches$nodeBirth, Q.rPBranches$nodeDeath),
                   edge.length = Q.rPBranches$tDeath - Q.rPBranches$tBirth,
                   tip.label = paste("t", 1:(1 + Q.nPBranches / 2), sep = ""),
                   root.edge = Q.Proot.edge, root.time = Q.Proot.time, nAlive = Q.nPAlive,
                   Hassoc = Q.rPBranches$Hassoc, root.Hassoc = Q.Proot.Hassoc)
    class(Q.Pphy)	<- "phylo"
...
#' Convert cophylogenetic trees from Ape's phylo format to the internal Branches
#' format
#'
#' The following function converts a phylo host-(dual) parasite tree into
#' internal Branches format
#' @param cophy a cophylogeny (in phylo format) containing one host and two
...
convert_HPQCophyloToBranches<-function(cophy) {
  ...
  P.PBranches <- data.frame(alive = rep(NA, nrow(cophy[[2]]$edge)), nodeBirth = cophy[[2]]$edge[,1],
                            tBirth = NA, nodeDeath = cophy[[2]]$edge[, 2], tDeath = NA, Hassoc = NA,
                            branchNo = 2:(nrow(cophy[[2]]$edge) + 1))

  P.ancBranches <- match(P.PBranches$nodeBirth, P.PBranches$nodeDeath)

  P.PBranches$tBirth <- sapply(1:length(P.PBranches$nodeBirth), get_tBirth, cophy[[2]]$root.edge,
                               cophy[[2]]$edge.length, ancBranches = P.ancBranches) + cophy[[2]]$root.time
  ...
  P.PBranches <- rbind(data.frame(alive = NA, nodeBirth = 0, tBirth = cophy[[2]]$root.time,
                                  nodeDeath = P.rootNode, tDeath = cophy[[2]]$root.time + cophy[[2]]$root.edge,
                                  Hassoc = cophy[[2]]$root.Hassoc, branchNo = 1), P.PBranches) # adding the root
  ...
  Q.PBranches <- data.frame(alive = rep(NA, nrow(cophy[[3]]$edge)), nodeBirth = cophy[[3]]$edge[, 1],
                            tBirth = NA, nodeDeath = cophy[[3]]$edge[, 2], tDeath = NA, Hassoc = NA,
                            branchNo = 2:(nrow(cophy[[3]]$edge) + 1))

  Q.ancBranches <- match(Q.PBranches$nodeBirth, Q.PBranches$nodeDeath)

  Q.PBranches$tBirth <- sapply(1:length(Q.PBranches$nodeBirth), get_tBirth, cophy[[3]]$root.edge,
                               cophy[[3]]$edge.length, ancBranches = Q.ancBranches) + cophy[[3]]$root.time
  ...
  Q.PBranches <- rbind(data.frame(alive = NA, nodeBirth = 0, tBirth = cophy[[3]]$root.time,
                                  nodeDeath = Q.rootNode, tDeath = cophy[[3]]$root.time + cophy[[3]]$root.edge,
                                  Hassoc = cophy[[3]]$root.Hassoc, branchNo = 1), Q.PBranches) # adding the root
