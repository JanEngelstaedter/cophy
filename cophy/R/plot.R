...
plot.cophylogeny <- function(x, ParasiteCol = c("Red", "Blue"), ResistanceCol = "lawn green",
  ...
  if (class(cophy[[length(cophy)]]) == "phylo") {
    TraitTracking <- NA
    if (plotHResistance == "TRUE")
      warning("Cophy object does not contain host resistance information")
  ...
  if (length(cophy) >= 3) {
    if (plotPEvolution[[2]] == TRUE & class(cophy[[3]]) == "phylo") {
      ...
      if (length(Qphy$edge[, 1]) > 2) {
        for (i in (Q.firstPNode + 1):Q.noPNodes) {
          # loop covering all internal nodes
          ...
          Q.PBranchLines <- rbind(Q.PBranchLines, c(tnew, tnew + Qphy$edge.length[Q.daughterBranches[1]],
                                                    HBranchLines[Qphy$Hassoc[Q.daughterBranches[1]], 3]))
          Q.PBranchLines <- rbind(Q.PBranchLines, c(tnew, tnew + Qphy$edge.length[Q.daughterBranches[2]],
                                                    HBranchLines[Qphy$Hassoc[Q.daughterBranches[2]], 3]))
        }
      }


      for (i in Q.firstPNode:Q.noPNodes) {
        # loop covering all internal P nodes
        ...
        Q.PConnectorLines <- rbind(Q.PConnectorLines, c(tnew, Q.PBranchLines[daughterBranches[1],
                                                                             3], Q.PBranchLines[daughterBranches[2], 3], hostJump))
      }
    }
  }
  ...
  if (!is.null(Hphy$root.edge)) {
    ...
    if (plotQ == TRUE) {
      Q.PBranchLines <- t(t(Q.PBranchLines) + c(Qphy$root.edge, Qphy$root.edge,
                                                0))
      if (is.null(Qphy$root.Hassoc))
        Q.Proot.y <- HBranchLines[1, 3] else Q.Proot.y <- HBranchLines[Qphy$root.Hassoc, 3]
        Q.PBranchLines <- rbind(c(0, Qphy$root.edge, Q.Proot.y), Q.PBranchLines)
        Q.PConnectorLines <- t(t(Q.PConnectorLines) + c(Qphy$root.edge, 0, 0,
                                                        0))
        ...
        if (length(Q.PConnectorLines[, 1]) > 1) {
          Q.PConnectorLines[, 1:3] <- sweep(Q.PConnectorLines[, 1:3], 2, -c(Q.xshift,
                                                                            Q.yshift, Q.yshift))
        } else {
          Q.PConnectorLines[1, 1:3] <- Q.PConnectorLines[1, 1:3] + c(Q.xshift,
                                                                     Q.yshift, Q.yshift)
        }
    }
  }
  ...
  if (!is.na(TraitTracking)) {
    ...
    for (i in 1:length(TraitTracking)) {
      if (nrow(TraitTracking[[i]]) == 1) {
        greenLines <- rbind(greenLines, c(TraitTracking[[i]][[1, 1]], max(HBranchLines$x2),
                                          HBranchLines[i, 3]))
      } else {
        for (j in 1:(nrow(TraitTracking[[i]]) - 1)) {
          if (TraitTracking[[i]][[j, 2]] == 1) {
            greenLines <- rbind(greenLines, c(TraitTracking[[i]][[j, 1]],
                                              TraitTracking[[i]][[j + 1, 1]], HBranchLines[i, 3]))
          }
        }
      }
    }
    for (i in unique(c(greenLines[, 1], greenLines[, 2]))) {
      # loop covering all internal nodes
      ...
      if (length(daughterBranches) == 2) {
        ...
        greenConnections <- rbind(greenConnections, c(tnew, greenLines[daughterBranches[1],
                                                                       3], greenLines[daughterBranches[2], 3]))
      }
    }
  }
  ...
  graphics::plot.new()
  graphics::plot.window(xlim = c(0, max(HBranchLines[, 2])), ylim = c(0, max(HBranchLines[,
                                                                                          3])))
  for (i in 1:length(HBranchLines[, 1])) graphics::lines(c(HBranchLines[i, 1],
                                                           HBranchLines[i, 2]), c(HBranchLines[i, 3], HBranchLines[i, 3]))
  for (i in 1:length(HConnectorLines[, 1])) graphics::lines(c(HConnectorLines[i,
                                                                              1], HConnectorLines[i, 1]), c(HConnectorLines[i, 2], HConnectorLines[i, 3]))

  if (class(TraitTracking) == "list") {
    for (i in 1:nrow(greenLines)) graphics::lines(c(greenLines[i, 1], greenLines[i,
                                                                                 2]), c(greenLines[i, 3], greenLines[i, 3]), col = ResistanceCol)
    for (i in 1:length(greenConnections[, 1])) graphics::lines(c(greenConnections[i,
                                                                                  1], greenConnections[i, 1]), c(greenConnections[i, 2], greenConnections[i,
                                                                                                                                                          3]), col = ResistanceCol)
  }
  if (plotP == TRUE) {
    for (i in 1:length(PBranchLines[, 1])) graphics::lines(c(PBranchLines[i,
                                                                          1], PBranchLines[i, 2]), c(PBranchLines[i, 3], PBranchLines[i, 3]), col = ParasiteCol[[1]])
    for (i in 1:length(PConnectorLines[, 1])) {
      if (PConnectorLines[i, 4] == TRUE)
        graphics::arrows(PConnectorLines[i, 1], PConnectorLines[i, 2], PConnectorLines[i,
                                                                                       1], PConnectorLines[i, 3], col = ParasiteCol[[1]], length = 0.1,
                         angle = 10) else graphics::lines(c(PConnectorLines[i, 1], PConnectorLines[i, 1]),
                                                          c(PConnectorLines[i, 2], PConnectorLines[i, 3]), col = ParasiteCol[[1]])
    }
  }
  if (plotQ == TRUE) {
    for (i in 1:length(Q.PBranchLines[, 1])) {
      graphics::lines(c(Q.PBranchLines[i, 1], Q.PBranchLines[i, 2]), c(Q.PBranchLines[i,
                                                                                      3], Q.PBranchLines[i, 3]), col = ParasiteCol[[2]])
    }
    for (i in 1:length(Q.PConnectorLines[, 1])) {
      if (Q.PConnectorLines[i, 4] == TRUE) {
        graphics::arrows(Q.PConnectorLines[i, 1], Q.PConnectorLines[i, 2],
                         Q.PConnectorLines[i, 1], Q.PConnectorLines[i, 3], col = ParasiteCol[[2]],
                         length = 0.1, angle = 10)
      } else {
        graphics::lines(c(Q.PConnectorLines[i, 1], Q.PConnectorLines[i, 1]),
                        c(Q.PConnectorLines[i, 2], Q.PConnectorLines[i, 3]), col = ParasiteCol[[2]])
      }
    }
  }
}
