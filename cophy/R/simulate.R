...
simulate_HostTrees <- function(tmax, lambda, mu, K, timestep, reps, filename = NA) {
  ...
  for(i in 1:reps) {
    ...
    if ((reps <= 20) | ((reps > 20) & (reps <= 50) & (i%%5 == 0)) |
        ...
        ((reps > 100) & (reps <= 500) & (i%%50 == 0)) | ((reps > 500) & (i%%100 == 0)))
...
simulate_cophys_HP <- function(tmax, lambda, mu, beta, gamma, sigma, nu, kappa, delta,
                               K, timestep, reps, filename = NA) {
  times             <- list(start = NA, end = NA, duration = NA)
  times[[1]]        <- Sys.time()
  parameters        <- c(tmax, lambda, mu, beta, gamma, sigma, nu, kappa, delta, K, timestep)
  names(parameters) <- c("tmax", "lambda", "mu", "beta", "gamma", "sigma", "nu", "kappa", "delta", "K", "timestep")
  all.trees         <- list() # an empty list that will later contain all the trees
  stats             <- matrix(NA, nrow = reps, ncol = 4)
  colnames(stats)   <- c("NoHspecies", "NoPspecies", "FractionInfected", "MeanNoInfections")
  for(i in 1:reps) {
    cophy <- rcophylo_HP(tmax = tmax, lambda = lambda, mu = mu, beta = beta, gamma = gamma, sigma = sigma,
                         nu = nu, kappa = kappa, delta = delta, timestep = timestep, K = K)
    all.trees[[i]]  <- cophy
    stats[i, ]      <- get_infectionStatistics(cophy)
    if ((reps <= 20) | ((reps > 20) & (reps <= 50) & (i%%5 == 0)) | ((reps > 50) & (reps <= 100) & (i%%10 == 0)) |
        ((reps > 100) & (reps <= 500) & (i%%50 == 0)) | ((reps > 500) & (i%%100 == 0)))
      print(paste("Replicate", i, "finished!"))
  }
  ...
  output <- list("parameters" = parameters, "replications" = reps, "Trees" = all.trees, "statistics" = stats, "times" = times)
  save(output, file = paste(filename, ".RData", sep = ""))
  stats
}
...
simulate_cophys_PonH <- function(Htrees, fromHtree = NA, toHtree = NA, HtreesPhylo = NA, P.startT, beta, gamma,
                                 sigma, nu, kappa, delta, timestep, reps1 = 1, reps2 = 1, filename = NA, ncores = 1)
{
  ...
  for(i0 in fromHtree:toHtree) {
    ...
    Ptrees[(i+1):(i+reps1*reps2)] <- foreach::foreach(i12 = 1:(reps1 * reps2), .export = c('rcophylo_PonH', 'convert_PBranchesToPhylo'), .packages = "ape") %dopar% {
      ...
      rcophylo_PonH(tmax = tmax, H.tree = Htrees[[i0]], beta = beta, gamma = gamma, sigma = sigma, nu = nu,
                    kappa = kappa, delta = delta, P.startT = P.startT, ini.Hbranch = ini.HBranches[i2],
                    timestep = timestep, Gdist = Gdist[[i0]], export.format = "PhyloPonly")
    }
    ...
    for(i1 in 1:reps2) {
      for(i2 in 1:reps1) {
        i <- i + 1
        stats[i, ] <- c(i0, i, ini.HBranches[i2], i1, get_infectionStatistics(list(HtreesPhylo[[i0]], Ptrees[[i]])))
      }
    }
    ...
    output <- list("parameters" = parameters, "replicates" = list("nHtrees" = nHtrees, "reps1" = reps1, "reps2" = reps2),
                   "Htrees" = HtreesPhylo, "Ptrees" = Ptrees, "statistics" = stats, "times" = times)
    save(output, file = paste(filename, ".RData", sep = ""))
...
simulate_cophys_PQonH <- function(Htrees, fromHtree = NA, toHtree = NA, P.startT, beta, gamma.P, gamma.Q,
                                  sigma.self, sigma.cross, nu.P, nu.Q, kappa.P, kappa.Q, delta.P, delta.Q,
                                  timestep, reps1 = 1, reps2 = 1, filename = NA, ncores = 1) {
  ...
  parameters <- c(tmax, P.startT, beta, gamma.P, gamma.Q, sigma.self, sigma.cross, nu.P, nu.Q, kappa.P,
                  kappa.Q, delta.P, delta.Q, timestep)
  names(parameters) <- c("tmax", "P.startT", "beta", "gamma.P", "gamma.Q", "sigma.self", "sigma.cross",
                         "nu.P", "nu.Q", "kappa.P", "kappa.Q", "delta.P", "delta.Q", "timestep")
  ...
  colnames(stats) <- c("HTreeNo", "PTreeNo", "IniHBranch", "Rep", "noHspecies", "P.NoPspecies",
                       "Q.NoPspecies", "P.fractionInfected", "Q.fractionInfected", "PandQ.fractionHinfected",
                       "P.meanInfectionLevel", "Q.meanInfectionLevel", "Total.meanInfection")
  ...
  for(i0 in fromHtree:toHtree) {
    ...
    Ptrees[(i+1):(i+reps1*reps2)] <- foreach::foreach(i12 = 1:(reps1 * reps2),
                                            .export = c('rcophylo_PQonH', 'convert_PQBranchesToPhylo',
                                                        "convert_HBranchesToPhylo"), .packages = "ape") %dopar% {
      i1 <- (i12 - 1) %/% reps1 + 1 # creating a counter for the relpicate number
      i2 <- ((i12 - 1) %% reps1) + 1 # creating a counter for the starting time point
      rcophylo_PQonH(tmax = tmax, H.tree = Htrees[[i0]], beta = beta, gamma.P = gamma.P, gamma.Q = gamma.Q,
                     sigma.self = sigma.self, sigma.cross = sigma.cross, nu.P = nu.P, nu.Q = nu.Q,
                     kappa.P = kappa.P, kappa.Q = kappa.Q, delta.P = delta.P, delta.Q = delta.Q,
                     P.startT = P.startT, ini.Hbranch = ini.HBranches[i2], timestep = timestep,
                     Gdist = Gdist[[i0]], export.format = "PhyloPonly")
    }
...
simulate_cophys_PonH_Htrait <- function(Htrees, HtreesPhylo = NA, fromHtree = NA, toHtree = NA, beta = 0.1,
  ...
  parameters <- c(tmax, P.startT, beta, gamma, sigma, nu, kappa, delta, epsilon.1to0, epsilon.0to1,
                  initialTrait, omega, rho, psi, reps1, reps2, timestep)
  names(parameters) <- c("tmax", "P.startT", "beta", "gamma", "sigma", "nu", "kappa", "delta", "epsilon.1to0",
                         "epsilon.0to1", "startTrait", "omega", "rho", "psi", "reps1", "reps2", "timestep")
  ...
  if (class(TraitTracking) == "logical") {
    TraitTracking <-foreach::foreach(i0 = fromHtree:toHtree, .export = c('get_preInvasionTraits'),.packages = "ape") %dopar% {
      get_preInvasionTraits(H.tree = Htrees[[i0]], P.startT = P.startT, epsilon.1to0 = epsilon.1to0,
                            epsilon.0to1 = epsilon.0to1, startTrait = startTrait, timestep = timestep)
    }
  }
  ...
  for(i0 in fromHtree:toHtree) {
    ...
    if (any(is.na(ini.Hbranch))) {
    	IniBranch <- sample(Htrees[[i0]][, 6][which(Htrees[[i0]][, 5] >= P.startT & Htrees[[i0]][, 3] <= P.startT)], reps1)
    } else if (length(ini.Hbranch) != length(fromHtree:toHtree) * reps1) {
    	stop("Number of provided initial branches does not match the requested number of initial branches.")
    } else {
    	IniBranch <- ini.Hbranch[(((i0 - fromHtree) * reps1) + 1):(((i0 - fromHtree) * reps1) + reps1)]
    }

    if (class(Htrees[[i0]]) == "data.frame") {
    	# parallel loop for running the simulations:
	    Ptrees[(i + 1):(i + reps1 * reps2)] <- foreach::foreach(i12 = 1:(reps1 * reps2),
	                                                 .export = c('rcophylo_PonH_Htrait', 'convert_PBranchesToPhylo'), .packages="ape") %dopar% {
    	  i1 <- (i12 - 1) %/% reps1 + 1 # creating a counter for the relpicate number
	    	i2 <- ((i12 - 1) %% reps1) + 1 # creating a counter for the starting time point
    	  rcophylo_PonH_Htrait(tmax = tmax, H.tree = Htrees[[i0]], beta = beta, gamma = gamma, sigma = sigma,
    	                       nu = nu, kappa = kappa, delta = delta, epsilon.1to0 = epsilon.1to0,
    	                       epsilon.0to1 = epsilon.0to1, startTrait = startTrait, omega = omega, rho = rho,
    	                       psi = psi, TraitTracking = TraitTracking[[i0]], prune.extinct = FALSE,
    	                       export.format = "PhyloPonly", P.startT = P.startT, ini.Hbranch = IniBranch[i1],
    	                       Gdist=Gdist[[i0]], timestep=timestep)
    	}
    } else if (class(Htrees[[i0]]) == "big.matrix") {
		  # parallel loop for running the simulations:
	    Ptrees[(i + 1):(i + reps1 * reps2)] <- foreach::foreach(i12 = 1:(reps1 * reps2),
	                                                   .export = c('rcophylo_PonH_Htrait', 'convert_PBranchesToPhylo'),
	                                                   .packages = c("ape", "bigmemory")) %dopar% {
	      ...
	      i2 <- ((i12 - 1) %% reps1) + 1 # creating a counter for the starting time point, ranges from 1:reps1
	         rcophylo_PonH_Htrait(tmax = tmax, H.tree = bigmemory::attach.big.matrix(descripTrees[[i0]]),
	                               beta = beta, gamma = gamma, sigma = sigma, nu = nu, kappa = kappa,
	                               delta = delta, epsilon.1to0 = epsilon.1to0, epsilon.0to1 = epsilon.0to1,
	                               startTrait = startTrait, omega = omega, rho = rho, psi = psi,
	                               TraitTracking = TraitTracking[[i0]], prune.extinct = FALSE,
	                               export.format = "PhyloPonly", P.startT = P.startT, ini.Hbranch = IniBranch[i2],
	                              Gdist = Gdist[[i0]], timestep = timestep)
    	 }
    }
    ...
    for(i1 in 1:reps2) { #cycling through starting timepoint
      for(i2 in 1:reps1) { #cycling through reps
        i <- i + 1
        stats[i, ] <- c(i0, i, IniBranch[i2], i1, get_infectionStatistics(list(HtreesPhylo[[i0 - (fromHtree - 1)]], Trees[[i]])))
      }
    }
    times[[2]] <- Sys.time()
    times[[3]] <- times[[2]] - times[[1]]
    ...
    output <- list("parameters" = parameters, "replicates" = list("nHtrees" = nHtrees, "reps1" = reps1, "reps2" = reps2),
                   "Htrees" = HtreesPhylo, "Ptrees" = Trees, "HResistanceTraits" = Traits, "statistics" = stats, "times" = times)
    save(output, file = paste(filename, ".RData", sep = ""))
    print(paste("        Simulations for host tree",i0,"finished!"))
  }
  parallel::stopCluster(cluster)
